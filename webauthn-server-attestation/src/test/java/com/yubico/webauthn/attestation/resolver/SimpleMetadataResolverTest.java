...
public class SimpleMetadataResolverTest {
    ...
    private final X509Certificate trustedCertificate = CertificateParser.parsePem(
        WebAuthnCodecs.json()
            .readValue(METADATA_JSON, MetadataObject.class)
            .getTrustedCertificates()
            .get(0)
    );

    public SimpleMetadataResolverTest() throws IOException, CertificateException {
    }

    @Test
    public void testResolve() throws Exception {
        final SimpleMetadataResolver resolver = SimpleMetadataResolver.fromMetadataJson(METADATA_JSON);
        MetadataObject metadata = resolver.resolve(trustedCertificate).orElse(null);
    ...
    public void resolveReturnsEmptyOnUntrustedSignature() throws Exception {
        final SimpleMetadataResolver resolver = new SimpleMetadataResolver(Collections.emptyList());

        assertEquals(Optional.empty(), resolver.resolve(trustedCertificate));
    }
