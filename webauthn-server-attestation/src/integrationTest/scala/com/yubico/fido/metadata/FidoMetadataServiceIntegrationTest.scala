...
class FidoMetadataServiceIntegrationTest
  ...
  describe("FidoMetadataService") {
    ...
    describe("downloaded with default settings") {
      ...
      describe("by AAGUID") {
        describe("correctly identifies") {
          def check(
            ...
            entry.get.getMetadataStatement.asScala should not be None
            entry.get.getMetadataStatement.get.getDescription.asScala should equal(
              Some(expectedDescription)
            )
            entry.get.getMetadataStatement.get.getAttachmentHint.asScala
      ...
      describe("by attestation certificate key identifier") {
        describe("correctly identifies") {
          def check(
              expectedDescription: String,
              testData: RealExamples.Example,
