...
- Methods `requireResidentKey(boolean)` and `isRequireResidentKey()`
  in `AuthenticatorSelectionCriteria` have been replaced
  by `residentKey(ResidentKeyRequirement)` and `getResidentKey()`, respectively.
+
...
The `MetadataService` interface has been replaced with `AttestationTrustSource`.
...
- `MetadataService` implementations were expected to validate
  ...
  The `RelyingParty.finishRegistration` method will perform
  certificate path validation internally
  and report the result via `RegistrationResult.isAttestationTrusted()`.
...
See the JavaDoc for `AttestationTrustSource` for details on how to implement it,
and see the `FidoMetadataService` class in the
link:../webauthn-server-attestation[`webauthn-server-attestation` module]
