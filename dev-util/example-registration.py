...
def make_signature_base(application_parameter_hex, challenge_parameter_hex, key_handle_hex, user_public_key_hex):
...
def sign(private_key, message_hex):
  return private_key.sign(unhexlify(message_hex), sig_alg)

def make_registration_request_message(user_private_key, user_public_key_hex, key_handle_hex, attestation_certificate_hex, application_parameter_hex, challenge_parameter_hex):
  return '05' + user_public_key_hex + hex(len(key_handle_hex) // 2)[2:] + key_handle_hex + attestation_certificate_hex + sign(user_private_key, make_signature_base(application_parameter_hex, challenge_parameter_hex, key_handle_hex, user_public_key_hex)).hex()
...
challenge_parameter_hex = '4142d21c00d94ffb9d504ada8f99b721f4b191ae4e37ca0140f696b6983cfacb'
...
example_signature_base_hex = make_signature_base(application_parameter_hex, challenge_parameter_hex, key_handle_hex, public_user_key_hex)
...
different_application_parameter_hex = 'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff'
message = make_registration_request_message(private_user_key, public_user_key_hex, key_handle_hex, attestation_certificate_hex, different_application_parameter_hex, challenge_parameter_hex)

print("Private key:", private_user_key_hex)
print("Public key:", public_user_key_hex)
print("Application parameter:", different_application_parameter_hex)
print("Registration request message:", message)
