...
object JavaGenerators {
  ...
  implicit def arbitraryList[A](implicit
      a: Arbitrary[List[A]]
  ): Arbitrary[java.util.List[A]] =
    Arbitrary(a.arbitrary map (l => new java.util.ArrayList[A](l.asJava)))

  implicit def arbitraryMap[A, B](implicit
      a: Arbitrary[Map[A, B]]
  ): Arbitrary[java.util.Map[A, B]] =
    Arbitrary(a.arbitrary map (m => new java.util.HashMap[A, B](m.asJava)))

  implicit def arbitrarySet[A](implicit
      a: Arbitrary[Set[A]]
  ): Arbitrary[java.util.Set[A]] =
    Arbitrary(a.arbitrary map (s => new java.util.HashSet[A](s.asJava)))

  implicit def arbitrarySortedSet[A](implicit
      a: Arbitrary[Set[A]]
  ): Arbitrary[java.util.SortedSet[A]] =
  ...
  def url(
    ...
    for {
      ...
      s"${scheme}://${host}${if (path.isEmpty) "" else "/"}${path}"
    )
