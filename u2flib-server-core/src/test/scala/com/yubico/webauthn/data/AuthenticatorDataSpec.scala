...
class AuthenticatorDataSpec extends FunSpec with Matchers {
  ...
  describe("AuthenticatorData") {
    ...
    def generateTests(authDataHex: HexString, hasAttestation: Boolean = false, hasExtensions: Boolean = false): Unit = {
      ...
      if (hasAttestation) {
        it("gets the correct attestation data from the raw bytes.") {
          asScalaOption(authData.attestationData) shouldBe defined
          ...
          asScalaIterator(authData.attestationData.get.credentialPublicKey.fieldNames).toSet should equal (Set("alg", "x", "y"))
      ...
      if (hasExtensions) {
        it("gets the correct extension data from the raw bytes.") {
          asScalaOption(authData.extensions) shouldBe defined
    ...
    describe("with both attestation data abd extensions") {
      generateTests(
