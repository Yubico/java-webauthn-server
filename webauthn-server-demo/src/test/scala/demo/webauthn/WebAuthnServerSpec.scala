...
class WebAuthnServerSpec extends FunSpec with Matchers {
  ...
  describe("WebAuthnServer") {
    ...
    describe("authentication") {
      ...
      val credentialId: ArrayBuffer = RegistrationTestData.FidoU2f.BasicAttestation.response.getRawId.toVector
      ...
      def newServerWithAuthenticationRequest(testData: RegistrationTestData) = {
        ...
        when(userStorage.getUserHandleForUsername(testData.userId.getName)).thenReturn(Some(testData.userId.getIdBase64).asJava)
        when(userStorage.lookup(testData.response.getId, testData.userId.getIdBase64)).thenReturn(Some(new RegisteredCredential(
          testData.response.getRawId,
  ...
  private def makeUserStorage(testData: RegistrationTestData) = {
    ...
    val registrations = util.Arrays.asList(CredentialRegistration.builder()
      ...
      .registration(RegistrationResult.builder()
        .keyId(new PublicKeyCredentialDescriptor(testData.response.getRawId))
        ...
        .publicKeyCose(testData.response.getResponse.getParsedAuthenticatorData.getAttestationData.get.getCredentialPublicKeyBytes.toArray)
