...
class AuthenticatorAttestationResponseSpec extends FunSpec with Matchers {
  ...
  describe("AuthenticatorAttestationResponse") {
    ...
    val exampleJson: ArrayBuffer =
      s"""{
          "authenticatorExtensions":{"boo":"${booExtension}"},
          "challenge":"${challenge}",
          "clientExtensions":{"foo":"${fooExtension}"},
          "origin":"${origin}",
          "tokenBinding":{"status":"${tokenBindingStatus.toJson}","id":"${tokenBindingId}"},
          "type":"webauthn.get"
    ...
    describe("has a clientDataJSON field which") {
      ...
      it("can be parsed as JSON.") {
        val clientData: JsonNode = new AuthenticatorAttestationResponse(U2fB64Encoding.encode(exampleAttestation), U2fB64Encoding.encode(exampleJson.toArray)).getClientData
      ...
      describe("defines attributes on the contained CollectedClientData:") {
        val response = new AuthenticatorAttestationResponse(U2fB64Encoding.encode(exampleAttestation), U2fB64Encoding.encode(exampleJson.toArray))
        ...
        it("challenge") {
          response.getCollectedClientData.getChallengeBase64 should equal (challenge)
        }
